...
public class Photo implements Comparable<Photo> {
	...
	public Photo(String photoSource, int width, int height, String date) {
		...
		this.height = height;
		this.date = date;
		if (photoSource == null || photoSource.isBlank() || date == null || date.isBlank() || width <= 0
				|| height <= 0) {
	...
	public String getComments() {
		if (!comments.equals(null) && comments.length() != 0) {
			comments.toString();
			return this.comments.substring(1, comments.length());
		} else {
			return "".trim();
		}
	...
	public int compareTo(Photo photo) {
		long date1 = Utilities.getDate(photo.date);
		long date2 = Utilities.getDate(date);
		int x;
		if (date2 < date1) {
			x = -1;
		} else if (date2 > date1) {
